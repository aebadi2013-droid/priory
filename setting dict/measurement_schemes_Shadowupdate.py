...
from shadowgrouping_v2.guarantees import (get_epsilon_Chebyshev_scalar_tighter_numba, get_epsilon_Chebyshev_scalar_numba, get_epsilon_Hoeffding_scalar_tighter_numba, get_epsilon_Hoeffding_scalar_numba, get_epsilon_Bernstein_scalar, get_epsilon_Bernstein_scalar_no_restricted_validity, get_epsilon_Bernstein_scalar_tighter_no_restricted_validity)
...
class Shadow_Grouping_Update6(Measurement_scheme):
    """ First find all cliques using DomClique then select the heaviest in each round
    """
    ...
    def find_setting(self,verbose=False):
        ...
        if np.any(self.N_hits == 0):
            for clique in self._cached_cliques:
                ...
                cliques_with_epsilon.append((self.total_hit_weight(weights, is_hit_candidate), setting_candidate))
            ...
            cliques_with_epsilon.append((self.total_hit_weight(weights, is_hit_candidate), shadowcliquesetting))
            #cliques_with_epsilon.append((self.total_hit_weight(weights, is_hit_candidate), setting_candidate))
            #self.N_hits += is_hit_candidate
            ...
            #self.N_hits -= is_hit_candidate
            cliques_with_epsilon.sort(key=lambda x: x[0], reverse = True)
            ...
            for clique in self._cached_cliques:
                ...
                self.N_hits += is_hit_candidate
                cliques_with_epsilon.append((self.get_epsilon_Bernstein_no_restricted_validity_v3(delta), setting_candidate))
                #cliques_with_epsilon.append((get_epsilon_Bernstein_scalar_no_restricted_validity(delta, self.N_hits, self.w, split=False), setting_candidate))
                self.N_hits -= is_hit_candidate #ggggg end
            ...
            #cliques_with_epsilon.append((self.total_hit_weight(weights, is_hit_candidate), setting_candidate))
            self.N_hits += is_hit_candidate
            cliques_with_epsilon.append((self.get_epsilon_Bernstein_no_restricted_validity_v3(delta), shadowcliquesetting))
            #cliques_with_epsilon.append((get_epsilon_Bernstein_scalar_no_restricted_validity(delta, self.N_hits, self.w, split=False), shadowcliquesetting))
            #cliques_with_epsilon.append((self.get_Bernstein_bound(), shadowcliquesetting))
            #print("epsilon for shadow clique is",self.get_epsilon_Bernstein_no_restricted_validity(delta))
            self.N_hits -= is_hit_candidate
...
class Shadow_Grouping_Update7(Measurement_scheme):
    """ 
        Returns p and a dictionary info holding further details on the matching procedure.
    """
    ...
    def find_setting(self,verbose=False):
        """ Finds the next measurement setting. Can be verbosed to gain further information during the procedure. """
        # sort observable list by respective weight
        settings_int, settings_reps = self.to_numba_format()
        ...
        if np.any(self.N_hits == 0):
            ...
            #         # break sequence is case all identities in setting are exhausted
                    if np.min(setting) > 0:
                        ...
                        if not any((existing == setting).all() for existing in self.clean_setting_cache[center_node]):
...
#print("shadow regime ended")
            ...
            for setting_candidate in self.clean_setting_cache[center_node]:
                    ...
                    cliques_with_epsilon.append((self.total_hit_weight(weights, is_hit_candidate), setting_candidate))
                    delta = 0.33
                    #print("just checking", get_epsilon_Chebyshev_scalar_tighter_numba(delta, self.N_hits, self.w, settings_int, settings_reps, self.obs, split=False)[0])
                    #self.N_hits += is_hit_candidate
                    #cliques_with_epsilon.append((self.get_epsilon_Bernstein_no_restricted_validity_v3(delta), setting_candidate))
                    #cliques_with_epsilon.append((self.get_Bernstein_bound(), setting_candidate))
                    #self.N_hits -= is_hit_candidate #ggggg end
                ...
                # Instead of checking only for this center_node, check across all cached settings
                #for cached_settings in self.clean_setting_cache.values(): #gggggggggggggggggggggggggg
                    #for setting_candidate in cached_settings:   #gggggggggggggggggggg
                        #for setting_candidate in self.clean_setting_cache[center_node]:
                        #key = tuple(tuple(x) if isinstance(x, (list, np.ndarray)) else x for x in clique)
                        #is_hit_candidate = self.is_hit_clique_cache[key]
                        #is_hit_candidate = []  #gggggggggggggggggg
                        #is_hit_candidate = hit_by_batch_numba(self.obs , setting_candidate) #ggggggggggggggggg
                        #cliques_with_epsilon.append((self.total_hit_weight(weights, is_hit_candidate), setting_candidate)) #ggggggggggggg
                        #self.N_hits += is_hit_candidate
                        #cliques_with_epsilon.append((self.get_epsilon_Bernstein_no_restricted_validity_v2(delta), setting_candidate))
                        #cliques_with_epsilon.append((self.get_Bernstein_bound(), setting_candidate))
                        #cliques_with_epsilon.append((self.total_hit_weight(weights, is_hit_candidate), setting_candidate))
            ...
            #cliques_with_epsilon.append((self.get_epsilon_Bernstein_no_restricted_validity_v3(delta), shadowcliquesetting))
            #cliques_with_epsilon.append((self.get_Bernstein_bound(), shadowcliquesetting))
            #print("epsilon for shadow clique is",self.get_epsilon_Bernstein_no_restricted_validity_v3(delta))
            #print("epsilon for shadow clique is",self.total_hit_weight(weights, is_hit_candidate))
            #self.N_hits -= is_hit_candidate
            #if valid_cliques:
            ...
            #for setting_candidate in self.clean_setting_cache[center_node]:
                    ...
                    self.N_hits += is_hit_candidate
                    #cliques_with_epsilon.append((self.get_epsilon_Bernstein_no_restricted_validity(delta), setting_candidate))
                    #cliques_with_epsilon.append((self.get_epsilon_Bernstein_tighter_no_restricted_validity(delta, self.N_hits, self.w, self.settings_dict, self.obs), setting_candidate))
                    #cliques_with_epsilon.append((self.get_Bernstein_bound(), setting_candidate))
                    #cliques_with_epsilon.append((get_epsilon_Chebyshev_scalar_tighter_numba(delta, self.N_hits, self.w, settings_int, settings_reps, self.obs, split=False)[0], setting_candidate))
                    #cliques_with_epsilon.append((get_epsilon_Chebyshev_scalar_numba(delta, self.N_hits, self.w, split=False)[0], setting_candidate))
                    cliques_with_epsilon.append((get_epsilon_Hoeffding_scalar_numba(delta, self.N_hits, self.w, split=False)[0], setting_candidate))
                    #cliques_with_epsilon.append((get_epsilon_Bernstein_scalar_no_restricted_validity(delta, self.N_hits, self.w, split=False), setting_candidate))
                    #cliques_with_epsilon.append((get_epsilon_Hoeffding_scalar_tighter_numba(delta, self.N_hits, self.w, settings_int, settings_reps, self.obs, split=False)[0], setting_candidate))
                    self.N_hits -= is_hit_candidate #gggggg end
            #else: #ggggggggggggg
                #print("I already met this candidate")
                # Instead of checking only for this center_node, check across all cached settings
                #for cached_settings in self.clean_setting_cache.values(): #ggggggggggggggggg
                    #for setting_candidate in cached_settings: #gggggggggggggggggg
                        #for setting_candidate in self.clean_setting_cache[center_node]:
                        #key = tuple(tuple(x) if isinstance(x, (list, np.ndarray)) else x for x in clique)
                        #is_hit_candidate = self.is_hit_clique_cache[key]
                        #is_hit_candidate = [] #ggggggggggggggggg
                        #is_hit_candidate = hit_by_batch_numba(self.obs , setting_candidate) #gggggggggggggggg
                        #self.N_hits += is_hit_candidate #gggggggggggggggg
                        #cliques_with_epsilon.append((self.get_epsilon_Bernstein_no_restricted_validity(delta), setting_candidate)) #ggggggggggggggggggg
                        #cliques_with_epsilon.append((self.get_Bernstein_bound(), setting_candidate))
                        #self.N_hits -= is_hit_candidate #gggggggggggggggggg
            is_hit_candidate = []
            if not any((existing == shadowcliquesetting).all() for existing in self.clean_setting_cache[center_node]):
                    self.clean_setting_cache[center_node].append(shadowcliquesetting)
            #self.clean_setting_cache[center_node].append(shadowcliquesetting)
            ...
            #cliques_with_epsilon.append((self.total_hit_weight(weights, is_hit_candidate), setting_candidate))
            self.N_hits += is_hit_candidate
            #cliques_with_epsilon.append((self.get_epsilon_Bernstein_tighter_no_restricted_validity(delta, self.N_hits, self.w, self.settings_dict, self.obs), shadowcliquesetting))
            #cliques_with_epsilon.append((self.get_epsilon_Bernstein_no_restricted_validity(delta), shadowcliquesetting))
            #cliques_with_epsilon.append((self.get_Bernstein_bound(), shadowcliquesetting))
            #cliques_with_epsilon.append((get_epsilon_Chebyshev_scalar_tighter_numba(delta,self.N_hits,self.w,settings_int,settings_reps,self.obs,split=False)[0], shadowcliquesetting))
            #cliques_with_epsilon.append((get_epsilon_Chebyshev_scalar_numba(delta, self.N_hits, self.w, split=False)[0], shadowcliquesetting))
            cliques_with_epsilon.append((get_epsilon_Hoeffding_scalar_numba(delta, self.N_hits, self.w, split=False)[0], shadowcliquesetting))
            #cliques_with_epsilon.append((get_epsilon_Bernstein_scalar_no_restricted_validity(delta, self.N_hits, self.w, split=False), shadowcliquesetting))
            #cliques_with_epsilon.append((get_epsilon_Hoeffding_scalar_tighter_numba(delta,self.N_hits,self.w,settings_int,settings_reps,self.obs,split=False)[0], shadowcliquesetting))
            #print("epsilon for shadow clique is",self.get_epsilon_Bernstein_tighter_no_restricted_validity(delta, self.N_hits, self.w, self.settings_dict, self.obs))
            #print("epsilon for shadow clique is",self.get_epsilon_Bernstein_no_restricted_validity(delta))
            #print("epsilon for shadow clique is",get_epsilon_Chebyshev_scalar_tighter_numba(delta, self.N_hits, self.w, settings_int, settings_reps, self.obs, split=False)[0])
            #print("epsilon for shadow clique is",get_epsilon_Chebyshev_scalar_numba(delta, self.N_hits, self.w, split=False)[0])
            #print("epsilon for shadow clique is",get_epsilon_Hoeffding_scalar_numba(delta, self.N_hits, self.w, split=False)[0])
            #print("epsilon for shadow clique is",get_epsilon_Bernstein_scalar_tighter_no_restricted_validity(delta, self.N_hits, self.w, self.settings_dict, self.obs, split=False))
            #print("epsilon for shadow clique is",get_epsilon_Hoeffding_scalar_tighter_numba(delta, self.N_hits, self.w, settings_int, settings_reps, self.obs, split=False)[0])
            self.N_hits -= is_hit_candidate
            #if valid_cliques:
            #    self.clean_clique_cache[center_node] = valid_cliques
            #    self.clean_setting_cache[center_node] = valid_settings
