...
from shadowgrouping.guarantees import (
    get_epsilon_Hoeffding_scalar_tighter, get_epsilon_Hoeffding_scalar_tighter_numba)
...
def test_valid_input_no_repeated_settings():
    ...
    epsilon_1 = get_epsilon_Hoeffding_scalar_tighter(delta, N_hits, w, settings_dict, obs)
    epsilon_2, _ = get_epsilon_Hoeffding_scalar_tighter_numba(delta, N_hits, w, settings_int, 
                                                              settings_reps, obs)
...
def test_all_zero_hits():
    ...
    epsilon = get_epsilon_Hoeffding_scalar_tighter(delta, N_hits, w, settings_dict, obs)
...
def test_some_zero_hits():
    ...
    eps_stat_1 = get_epsilon_Hoeffding_scalar_tighter(delta, N_hits[3:], w[3:],
                                                      settings_dict, obs[3:])
    eps_stat_2, _ = get_epsilon_Hoeffding_scalar_tighter_numba(delta, N_hits[3:], w[3:],
...
def test_split():
    ...
    eps_stat_1, eps_sys_1 = get_epsilon_Hoeffding_scalar_tighter(delta, N_hits, w, settings_dict, 
                                                                 obs, split=True)
    eps_stat_2, eps_sys_2 = get_epsilon_Hoeffding_scalar_tighter_numba(delta, N_hits, w, settings_int, 
                                                                       settings_reps, obs, split=True)
    eps_sys_check = np.sum(np.abs(w[:3]))
    eps_stat_check_1 = get_epsilon_Hoeffding_scalar_tighter(delta, N_hits[3:], w[3:],
                                                            settings_dict, obs[3:])
    eps_stat_check_2, _ = get_epsilon_Hoeffding_scalar_tighter_numba(delta, N_hits[3:], w[3:],
                                                                     settings_int, settings_reps, obs[3:])
...
def test_invalid_deltas_raise_error(invalid_delta):
    ...
    with pytest.raises(ValueError, match="delta must be in the interval \\(0,1\\)"):
        get_epsilon_Hoeffding_scalar_tighter(invalid_delta, N_hits, w, settings_dict, obs)
