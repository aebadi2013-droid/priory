...
from shadowgrouping.guarantees import (
    get_epsilon_Hoeffding_scalar, get_epsilon_Hoeffding_scalar_numba)
...
def test_valid_input():
    ...
    epsilon_1 = get_epsilon_Hoeffding_scalar(delta, N_hits, w)
    epsilon_2, _ = get_epsilon_Hoeffding_scalar_numba(delta, N_hits, w)
...
def test_all_zero_hits():
    ...
    epsilon_1 = get_epsilon_Hoeffding_scalar(delta_input, N_hits_input, w_input)
    epsilon_2, _ = get_epsilon_Hoeffding_scalar_numba(delta_input, N_hits_input, w_input)
...
def test_some_zero_hits():
    ...
    eps_stat = get_epsilon_Hoeffding_scalar(delta_input, N_hits_input[3:], w_input[3:])
...
def test_split():
    ...
    eps_stat_1, eps_sys_1 = get_epsilon_Hoeffding_scalar(delta_input, N_hits_input, w_input, split=True)
    eps_stat_2, eps_sys_2 = get_epsilon_Hoeffding_scalar_numba(delta_input, N_hits_input, w_input, split=True)
    eps_sys_check = np.sum(np.abs(w_input[:3]))
    eps_stat_check = get_epsilon_Hoeffding_scalar(delta_input, N_hits_input[3:], w_input[3:])
...
def test_invalid_deltas_raise_error(invalid_delta):
    ...
    with pytest.raises(ValueError, match="delta must be in the interval \\(0,1\\)"):
        epsilon = get_epsilon_Hoeffding_scalar(invalid_delta, N_hits, w)
