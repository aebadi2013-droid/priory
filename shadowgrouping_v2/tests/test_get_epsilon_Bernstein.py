...
from shadowgrouping.guarantees import get_epsilon_Bernstein
...
def test_valid_input():
    delta_input = 0.05
    N_hits_input = np.array([10, 20, 30])
    w_input = np.array([1.0, 2.0, 3.0])
    
    epsilon = get_epsilon_Bernstein(delta_input, N_hits_input, w_input)
...
def test_some_zero_hits():
    ...
    eps_stat = get_epsilon_Bernstein(delta_input, N_hits_input[3:], w_input[3:])
...
def test_split():
    ...
    eps_stat, eps_sys = get_epsilon_Bernstein(delta_input, N_hits_input, w_input, split=True)
    eps_sys_check = np.sum(np.abs(w_input[:3]))
    eps_stat_check = get_epsilon_Bernstein(delta_input, N_hits_input[3:], w_input[3:])
...
def test_invalid_deltas_raise_error(invalid_delta):
    ...
    with pytest.raises(ValueError, match="delta must be in the interval \\(0,1\\)"):
        get_epsilon_Bernstein(invalid_delta, N_hits_input, w_input)
