...
from shadowgrouping.guarantees import get_epsilon_single_Hoeffding_plus_union_bound
...
def test_valid_input():
    ...
    epsilon = get_epsilon_single_Hoeffding_plus_union_bound(delta, N_hits, w)
...
def test_all_zero_hits():
    ...
    epsilon = get_epsilon_single_Hoeffding_plus_union_bound(delta_input, N_hits_input, w_input)
    assert epsilon == np.sum(np.abs(w_input))
    
def test_some_zero_hits():
    delta_input = 0.05
    N_hits_input = np.array([0, 0, 0, 10, 20])
    w_input = np.array([1.0, 2.0, 5.0, 10.0, 20.0])
    
    epsilon = get_epsilon_single_Hoeffding_plus_union_bound(delta_input, N_hits_input, w_input)
    eps_sys = np.sum(np.abs(w_input[:3]))
    eps_stat = get_epsilon_single_Hoeffding_plus_union_bound(delta_input, N_hits_input[3:], w_input[3:])
...
def test_split():
    ...
    eps_stat, eps_sys = get_epsilon_single_Hoeffding_plus_union_bound(delta_input, N_hits_input, w_input, split=True)
    eps_sys_check = np.sum(np.abs(w_input[:3]))
    eps_stat_check = get_epsilon_single_Hoeffding_plus_union_bound(delta_input, N_hits_input[3:], w_input[3:])
...
def test_invalid_deltas_raise_error(invalid_delta):
    ...
    with pytest.raises(ValueError, match="delta must be in the interval \\(0,1\\)"):
        get_epsilon_single_Hoeffding_plus_union_bound(invalid_delta, N_hits, w)