...
from shadowgrouping.guarantees import N_delta_Bernstein_no_restricted_validity
...
def test_very_small_delta(delta=1e-10):
    assert np.isfinite(N_delta_Bernstein_no_restricted_validity(delta))
    
def test_invalid_deltas_raise_error(invalid_delta):
    with pytest.raises(ValueError, match="delta must be in the interval \\(0,1\\)"):
        N_delta_Bernstein_no_restricted_validity(invalid_delta)
