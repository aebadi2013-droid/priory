...
class Shadow_Grouping_Update6(Measurement_scheme):
    ...
    def find_setting(self,verbose=False):
        ...
        settingslist = [s for _, s in cliques_with_epsilon]
        if np.min(setting) == 0:
        ...
        if np.min(setting) == 0:
            for idx in reversed(order):
                ...
                # break sequence is case all identities in setting are exhausted
                if np.min(setting) > 0:
                    print("completed setting using shadow method")
                    break
    ...
    def find_setting(self,verbose=False):
                ...
                for o in clique:
                    ...
                    if hit_by(o_arr, setting_candidate):
                        ...
                        if np.min(setting_candidate) > 0:
                            break
                if np.min(setting_candidate) == 0:
                    print("using setting list to complete setting")
                    cliques_with_epsilon.sort(key=lambda x: x[0])
                    settingslist = [s for _, s in cliques_with_epsilon]
                    for o in settingslist:
                        if verbose:
        ...
        if np.min(setting) == 0:
            for o in settingslist:
                ...
                if np.min(setting) > 0:
                    completecliques += 1
                    print("completed setting using setting list at the end")
                    break
            else:
                completecliques += 1
        ...
        if np.min(setting) == 0:
            for idx in reversed(order):
                ...
                if np.min(setting) > 0:
                    completecliques += 1
                    print("completed setting using shadow method at the end")
                    break
...
class Shadow_Grouping_Update77(Measurement_scheme):
    """ First find all cliques using DomClique then select the heaviest in each round
    """
    ...
    def __init__(self, observables, weights, epsilon, weight_function):
        ...
        for clique in self._cached_cliques:
            ...
            for o in clique:
                ...
                if hit_by(o_arr, setting_candidate):
                    mask = (o_arr != 0)
                    setting_candidate[mask] = o_arr[mask]
                    if np.all(setting_candidate != 0):
    ...
    def find_setting(self,verbose=False):
        ...
        """if np.any(self.N_hits == 0):
            for setting_candidate in self._cached_settings:
                ...
                cliques_with_epsilon.append((self.total_hit_weight(weights, is_hit_candidate), setting_candidate))
            cliques_with_epsilon.sort(key=lambda x: x[0], reverse = True)
            _, setting = cliques_with_epsilon[0]
        else:"""
        for setting_candidate in self._cached_settings:
            is_hit_candidate = []
            is_hit_candidate = hit_by_batch_numba(self.obs , setting_candidate)
            self.N_hits += is_hit_candidate
            cliques_with_epsilon.append((Guaranteed_accuracy(delta, self.N_hits, self.w, split=False), setting_candidate))
            self.N_hits -= is_hit_candidate
        cliques_with_epsilon.sort(key=lambda x: x[0])
        _, setting = cliques_with_epsilon[0]
