...
class AEQuODomClique(Shadow_Grouping):
        ...
        while uncovered_nodes:
            for clique in cliques_sorted:
                if uncovered_nodes & set(clique):
                    MaxCliques.append(sorted([int(node) for node in clique]))
...
def qubitwise_commutation_graph(P):
    """
    Generates the qubitwise commutation graph using NetworkX.

    Inputs:
        P - (pauli) - Pauli to check for qubitwise commutation relations
    Outputs:
        (networkx.Graph) - an edge is weighted 1 if the pair of Paulis qubitwise commute
    """
    p = P.paulis()
    ...
    # Add nodes
    G.add_nodes_from(range(p))  # Assuming nodes are indexed as 0,1,2,...

    # Add edges based on qubitwise commutation relations
    for i in range(p):
        for j in range(p):
            if i != j:
                weight = 1 - qubitwise_inner_product(P.a_pauli(i), P.a_pauli(j))
                if weight == 1:  # Only add edges when they qubitwise commute
